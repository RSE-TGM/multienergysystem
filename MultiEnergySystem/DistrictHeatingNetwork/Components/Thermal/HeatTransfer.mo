...
import MultiEnergySystem.DistrictHeatingNetwork.Types;
  ...
  model FlowPressureDependentHeatTransferCoefficient
    ...
    parameter Modelica.Units.SI.Pressure pnom_ht = p_nom  "Nominal pressure for heat transfer correlation (single tube)";
    parameter Modelica.Units.SI.PerUnit theta = 0.5 "Exponent in the pressure-dependency law";
    DistrictHeatingNetwork.Types.PerUnit p_over_pnom(start = 1) "Ratio between actual and nominap pressure";
    DistrictHeatingNetwork.Types.PerUnit p_pnom_reg  "Regularized ratio between actual and nominal pressure";
  initial equation
    assert(not computeTransport, "The model cannot use transport properties");
  equation

...
model GasBoilerTest
  ...
  MultiEnergySystem.DistrictHeatingNetwork.Sensors.IdealMassFlowSensor idealMassFlowSensor annotation (
    Placement(visible = true, transformation(origin={-30,-20},    extent={{-6,-6},
            {6,6}},                                                                                rotation = 0)));
...
equation
  connect(coldStartTSensor.outlet, expTankCold.inlet)
    annotation (Line(points={{44,-20},{72,-20}}, color={168,168,168}));
  connect(warmStartTSensor.outlet, expTankHot.inlet)
    annotation (Line(points={{42,20},{72,20}},
                                             color={168,168,168}));
  connect(massFlowSensor.outlet, warmBoiler.inlet)
    annotation (Line(points={{-24,20},{-10,20}},
                                               color={168,168,168}));
  connect(warmBoiler.outlet, warmStartTSensor.inlet)
    annotation (Line(points={{10,20},{22,20}},
                                             color={168,168,168}));
  ...
  connect(idealMassFlowSensor.outlet, coldBoiler.inlet)
    annotation (Line(points={{-24,-20},{-10,-20}}, color={168,168,168}));
  connect(coldBoiler.outlet, coldStartTSensor.inlet)
    annotation (Line(points={{10,-20},{24,-20}}, color={168,168,168}));

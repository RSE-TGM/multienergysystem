...
equation
  connect(pump.inlet, sourceP.outlet) annotation (
    Line(points={{-14.4,0},{-36,0}},        color = {140, 56, 54}));
  connect(omega.y, pump.in_omega) annotation (
    Line(points={{-39,50},{-7.2,50},{-7.2,9}},    color = {0, 0, 127}));
  connect(pump.outlet, cvalve.inlet) annotation (
    Line(points={{14.4,0},{18,0},{18,10},{26,10}},  color = {140, 56, 54}));
  ...
  connect(m_flow_set.y, sink.in_m_flow) annotation (
    Line(points={{61,50},{75,50},{75,15},{78,15}},          color = {0, 0, 127}));
  connect(theta.y, cvalve.opening) annotation (
    Line(points={{21,50},{34,50},{34,16.4}},      color = {0, 0, 127}));
  annotation (

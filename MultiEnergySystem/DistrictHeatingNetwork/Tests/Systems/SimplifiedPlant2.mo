...
model SimplifiedPlant2
  ...
  DistrictHeatingNetwork.Sources.VariableMassFlowPump m_flow_S100 annotation (
    Placement(transformation(origin = {-56, -25}, extent = {{6, -6}, {-6, 6}}, rotation = -90)));
  Modelica.Blocks.Sources.RealExpression m_flow_SP(y = 5*990/3600) annotation (
    Placement(transformation(origin = {-128, -5}, extent = {{88, -29}, {81, -21}}, rotation = -0)));
  H2GasFacility.Sources.SourcePressure sourceGas(redeclare model Medium = Gas, R = 1e-3, X0 = X_gas, computeEnergyVariables = true, computeTransport = false) annotation (
    Placement(transformation(origin = {-60, -75}, extent = {{7, -7}, {-7, 7}}, rotation = -90)));
equation
  ...
  connect(S100.outlet, m_flow_S100.inlet) annotation (
    Line(points = {{-56, -37}, {-56, -31}}, color = {140, 56, 54}));
  connect(m_flow_S100.outlet, S900.inlethot) annotation (
    Line(points = {{-56, -19}, {-56, -11}}, color = {140, 56, 54}));
  connect(S900.outletcold, S100.inlet) annotation (
    Line(points = {{-64, -11}, {-64, -37}}, color = {140, 56, 54}));
  connect(m_flow_SP.y, m_flow_S100.m_flow) annotation (
    Line(points = {{-47, -30}, {-52, -30}}, color = {0, 0, 127}));
  connect(sourceGas.outlet, S100.inletFuel) annotation (
    Line(points = {{-60, -68}, {-60, -61}}, color = {182, 109, 49}));
  annotation (

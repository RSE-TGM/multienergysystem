...
model Battery_powerdriven "Battery sink/source with a single port. Imposed power request"
...
equation
  ...
  der(SOC) =
    if P >= 0 then
      eta_chg * P / E_nom     // charging increases SOC
    else
      P / (eta_dis * E_nom);  // discharging decreases SOC

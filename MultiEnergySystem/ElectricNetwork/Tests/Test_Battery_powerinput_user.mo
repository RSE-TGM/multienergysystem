...
model Test_Battery_powerinput_user "Test with battery (charge/discharge) and a user both with prescribed power absoprtion"
  ...
  Components.ResistiveLine resistiveLine1(L = 1, A = 0.001, rho = 1.68e-5, useThermalPort = false)  annotation(
    Placement(transformation(origin = {42, 18}, extent = {{-10, -10}, {10, 10}})));
  Components.ResistiveLine resistiveLine2(L = 1, A = 0.001, rho = 1.68e-5, useThermalPort = false)  annotation(
  ...
  Components.Battery_powerinput battery_powerinput(E_nom = 1e6)  annotation(
    Placement(transformation(origin = {78, 18}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
equation

...
model IdealUser
  ...
  //parameter Types.MassFlowRate userdemand[:, 2] = [0, m_flow0; 24*3600, m_flow0] "Table for demand";
  ...
  Types.MassFlowRate m_flow_demand;
  ...
  Modelica.Blocks.Sources.TimeTable mfrDemandProfile(table=massflowratedemand)
    annotation (Placement(visible=true, transformation(
        origin={-50,40},
        extent={{-10,-10},{10,10}},
        rotation=0)));
  Modelica.Blocks.Sources.TimeTable energyDemandProfile(table=energydemand)
    "Energy profile in W"             annotation (Placement(visible=true, transformation(
        origin={-50,70},
        extent={{-10,-10},{10,10}},
        rotation=0)));
equation
  ...
  sink_demand.in_m_flow0 =if useEnergyDemand then energyDemandProfile.y*fluid.rho0/fluid.HHV_SCM_mix else mfrDemandProfile.y;

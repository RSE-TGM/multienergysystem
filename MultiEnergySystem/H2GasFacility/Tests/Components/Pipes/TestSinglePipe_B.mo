...
model TestSinglePipe_B
  ...
  MultiEnergySystem.H2GasFacility.Components.Pipes.Round1DFV roundPipe(
    ...
    Tin_start = Pipe.pipe1.Tin_start, Tout_start = Pipe.pipe1.Tout_start, rho_nom=Pipe.pipe1.rho_nom,
  ...
  MultiEnergySystem.H2GasFacility.Sources.SinkPressure sinkPressure(
    redeclare model Medium = Medium,
    R=1,
    p0(displayUnit="Pa") = 0.49e5,
    T0(displayUnit="K") = 20 + 273.15,
    X0=Xref,
    use_in_p0=true) annotation (Placement(transformation(extent={{56,-52},{76,-32}})));
...
equation
  ...
  connect(roundPipe.outlet, sinkPressure.inlet) annotation (Line(
      points={{22,0},{32,0},{32,-42},{56,-42}},
      color={182,109,49},
      thickness=0.5));
  connect(p_out.y, sinkPressure.in_p0) annotation (Line(points={{79,36},{84,36},{84,-6},{88,
          -6},{88,-33.6},{62,-33.6}}, color={0,0,127}));
  connect(sourceP.outlet, roundPipe.inlet) annotation (Line(
      points={{-40,0},{-22,0}},
      color={182,109,49},
      thickness=0.5));
  annotation (

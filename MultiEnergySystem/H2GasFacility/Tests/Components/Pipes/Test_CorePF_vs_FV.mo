...
model Test_CorePF_vs_FV
  "Base test model for a Plug Flow Core and Finite Volume Pipe"
  ...
  MultiEnergySystem.H2GasFacility.Components.Pipes.BaseClass.PlugFlowCore
    plugFlowCore( redeclare model Medium = Medium,
    L=Pipe.pipe1.L,
    H=Pipe.pipe1.h,
    t=Pipe.pipe1.t,
    pmax=Pipe.pipe1.pmax,
    ...
    X_start=Xref,
    Di=Pipe.pipe1.Di)
    annotation (Placement(transformation(extent={{-4,-76},{32,-40}})));
equation
  ...
  connect(m_flow.y, sink_PF.in_m_flow0)
    annotation (Line(points={{65,-10},{76,-10},{76,-53}}, color={0,0,127}));
  ...
  connect(sourceP_PF.outlet, plugFlowCore.inlet) annotation (Line(
      points={{-34,-58},{-4,-58}},
      color={182,109,49},
      thickness=0.5));
  connect(plugFlowCore.outlet, sink_PF.inlet) annotation (Line(
      points={{32,-58},{72,-58}},
      color={182,109,49},
      thickness=0.5));
  annotation (

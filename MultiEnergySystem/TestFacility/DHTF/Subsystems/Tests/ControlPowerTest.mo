...
model ControlPowerTest
  ...
  parameter Real Kp_TT7X1 = -0.001;//-0.001;
  ...
  Modelica.Blocks.Sources.RealExpression Pt_SP(y=if time < 5500 then 30E3 else 25e3)
                                                       annotation (Placement(transformation(extent={{-66,8},{-46,28}})));
  Modelica.Blocks.Sources.RealExpression Tout_SP(y=if time < 3000 then 65 + 273.15 else 60 + 273.15) annotation (Placement(transformation(extent={{-66,-26},{-46,-6}})));
equation
  ...
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(coordinateSystem(preserveAspectRatio=false)),
    experiment(StopTime= 9000, StartTime = 0, Tolerance = 1e-06, Interval = 0.5));

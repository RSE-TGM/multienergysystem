...
model CentralizedThreeGenGBEBCHPTES "Three heat generators + thermal energy storage"
  ...
  Subsystems.HeatGeneration.TES tES(q_m3h_S2=8) annotation (Placement(transformation(extent={{192,-50},{282,40}})));
  ...
  DistrictHeatingNetwork.Components.Pipes.RoundPipe1DFV PL_S200_rCD_hot(
    ...
    hctype=hctype) annotation (Placement(transformation(
        ...
        origin={256,70})));
equation
  connect(PL_S200_rCD_cold.outlet, tES.inlet) annotation (Line(
  ...
  connect(senthot, PL_S200_rCD_hot.outlet) annotation (Line(
      ...
      color={140,56,54},
      thickness=0.5));
  connect(PL_S200_rCD_hot.inlet, tES.outlet) annotation (Line(
      points={{256,60},{256,53.375},{255.45,53.375},{255.45,46.75}},
      color={140,56,54},
      thickness=0.5));
  connect(controlSignalBus.statusS200, tES.status) annotation (Line(
  ...
  connect(controlSignalBus.thetaFCV201, tES.theta) annotation (Line(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(controlSignalBus.omegaP201, tES.omega) annotation (Line(

within MultiEnergySystem.TestFacility.Data;
package ValveData
   "Data of RSE's District Heating Test Facility valves"
  constant MultiEnergySystem.TestFacility.Data.Valve TCV701(
    Name="TCV701",
    Tin_nom=67.73858 + 273.15 "Calculo teorico",
    q_nom_m3h=5,
    dp_nom=0.17e5,
    rho_nom=0.9792156e3 "Calculo teorico",
    Kv=12,
    Tin_start=36.7 + 273.15,
    pin_start=4.5e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve TCV711(
    Name="TCV711",
    Tin_nom=67.73858 + 273.15 "Calculo teorico",
    q_nom_m3h=5,
    dp_nom=0.17e5,
    rho_nom=0.9792156e3 "Calculo teorico",
    Kv=12,
    Tin_start=36.7 + 273.15,
    pin_start=4.5e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve TCV721(
    Name="TCV721",
    Tin_nom=67.73858 + 273.15 "Calculo teorico",
    q_nom_m3h=5,
    dp_nom=0.17e5,
    rho_nom=0.9792156e3 "Calculo teorico",
    Kv=12,
    Tin_start=36.7 + 273.15,
    pin_start=4.5e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve TCV731(
    Name="TCV731",
    Tin_nom=67.73858 + 273.15 "Calculo teorico",
    q_nom_m3h=5,
    dp_nom=0.17e5,
    rho_nom=0.9792156e3 "Calculo teorico",
    Kv=12,
    Tin_start=36.7 + 273.15,
    pin_start=4.5e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve TCV301(
    Name="TCV301",
    Tin_nom=8.721643 + 273.15 "Calculo teorico",
    q_nom_m3h=12,
    dp_nom=0.64e5,
    rho_nom=1.000016e3 "Calculo teorico",
    Kv=12,
    Tin_start=25 + 273.15,
    pin_start=4.5e5);
  constant MultiEnergySystem.TestFacility.Data.Valve TCV601(
    Name="TCV601",
    Tin_nom=86.17303 + 273.15 "Calculo teorico",
    q_nom_m3h=15,
    dp_nom=0.2e5,
    rho_nom=0.9680155e3 "Calculo teorico",
    Kv=33,
    Tin_start=15 + 273.15,
    pin_start=4.5e5);
  constant MultiEnergySystem.TestFacility.Data.Valve FCV101(
    Name="FCV101",
    Tin_nom=60 + 273.15,
    q_nom_m3h=15.60340167,
    dp_nom=2.5e5,
    rho_nom=0.9680155e3,
    Kv=33,
    Tin_start=60 + 273.15,
    pin_start=1.66e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve FCV201(
    Name="FCV201",
    Tin_nom=60 + 273.15,
    q_nom_m3h=4,
    dp_nom=2.5e5,
    rho_nom=0.9680155e3,
    Kv=33,
    Tin_start=60 + 273.15,
    pin_start=2e5);
  constant MultiEnergySystem.TestFacility.Data.Valve FCV401(
    Name="FCV401",
    Tin_nom=60 + 273.15,
    q_nom_m3h=8.8,
    dp_nom=1e5,
    rho_nom=0.9680155e3,
    Kv=33,
    Tin_start=60 + 273.15,
    pin_start=1.66e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.Linear);
  constant MultiEnergySystem.TestFacility.Data.Valve FCV901(
    Name="FCV901",
    Tin_nom=80 + 273.15,
    q_nom_m3h=19,
    dp_nom=1.5e5,
    rho_nom=985 "Calculo teorico",
    Kv=33,
    Tin_start=80 + 273.15,
    pin_start=3e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.SquareRoot);
  constant MultiEnergySystem.TestFacility.Data.Valve FCV701(
    Name="FCV701",
    Tin_nom=60 + 273.15,
    q_nom_m3h=4.15,
    dp_nom=1.035e5,
    rho_nom=985 "Calculo teorico",
    Kv=12,
    Tin_start=60 + 273.15,
    pin_start=3e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve FCV711(
    Name="FCV711",
    Tin_nom=60 + 273.15,
    q_nom_m3h=4.15,
    dp_nom=1.035e5,
    rho_nom=985 "Calculo teorico",
    Kv=12,
    Tin_start=60 + 273.15,
    pin_start=3e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve FCV721(
    Name="FCV721",
    Tin_nom=60 + 273.15,
    q_nom_m3h=4.15,
    dp_nom=1.035e5,
    rho_nom=985 "Calculo teorico",
    Kv=12,
    Tin_start=60 + 273.15,
    pin_start=3e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve FCV731(
    Name="FCV731",
    Tin_nom=60 + 273.15,
    q_nom_m3h=4.15,
    dp_nom=1.035e5,
    rho_nom=985 "Calculo teorico",
    Kv=12,
    Tin_start=60 + 273.15,
    pin_start=3e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve FCVC01(
    Name="FCVC01",
    Tin_nom=60 + 273.15,
    q_nom_m3h=4.15,
    dp_nom=1.035e5,
    rho_nom=985,
    Kv=33,
    Tin_start=60 + 273.15,
    pin_start=3e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve FCVC02(
    Name="FCVC02",
    Tin_nom=60 + 273.15,
    q_nom_m3h=4.15,
    dp_nom=1.035e5,
    rho_nom=985,
    Kv=33,
    Tin_start=80 + 273.15,
    pin_start=3e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
  constant MultiEnergySystem.TestFacility.Data.Valve FCVR01(
    Name="FCVR01",
    Tin_nom=30 + 273.15,
    q_nom_m3h=34,
    dp_nom=0.1e5,
    rho_nom=995,
    Kv=22,
    Tin_start=30 + 273.15,
    pin_start=2e5,
    openingChar=MultiEnergySystem.DistrictHeatingNetwork.Components.Types.valveOpeningChar.EqualPercentage);
end ValveData;

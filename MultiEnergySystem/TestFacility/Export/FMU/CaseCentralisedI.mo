...
model CaseCentralisedI
  ...
  Modelica.Blocks.Interfaces.RealOutput TT102(start = ToutGen_start) annotation (Placement(transformation(extent={{5,-80},{15,-70}}),    iconTransformation(
        ...
        rotation=0,
        origin={105,85})));
  Modelica.Blocks.Sources.RealExpression TT102_exp(y=facility.heatGeneration.S100.TT102.T_meas) annotation (Placement(transformation(extent={{-10,-80},{0,-70}})));
  Modelica.Blocks.Sources.RealExpression m_flow_S100_exp(y=facility.heatGeneration.S100.FT.m_flow) annotation (Placement(transformation(extent={{-10,-87.5},{0,-77.5}})));
  Modelica.Blocks.Interfaces.RealOutput FT101 annotation (Placement(transformation(extent={{5,-87.5},{15,-77.5}}),    iconTransformation(
        extent={{5,-5},{-5,5}},
        rotation=180,
        origin={105,75})));
...
equation
  connect(TT102_exp.y, TT102) annotation (Line(points={{0.5,-75},{10,-75}},    color={0,0,127}));
  ...
  connect(m_flow_S100_exp.y, FT101) annotation (Line(points={{0.5,-82.5},{10,-82.5}}, color={0,0,127}));
  annotation (

...
model ControlledElectricBoiler
  ...
  ElectricBoiler boiler annotation (Placement(transformation(extent={{-40,-40},{40,40}})));
  DistrictHeatingNetwork.Controllers.AWPIContinuous PI(Kp=Kp, Ti=Ti,
    Umax=omegamax,
    Umin=omegamin,
    y_start=boiler.m_flow_S4)
  ...
  Modelica.Blocks.Interfaces.RealInput theta "Opening valve of the valve's system" annotation (
      Placement(transformation(extent={{-100,20},{-60,60}}), iconTransformation(extent={{-100,20},{-60,
            60}})));
  ...
  Modelica.Blocks.Interfaces.RealInput Tout_SP "Outlet temperature set point" annotation (Placement(
        transformation(extent={{-100,-60},{-60,-20}}), iconTransformation(extent={{-100,-60},{-60,-20}})));
...
equation
  ...
  connect(Tout_SP, boiler.Tout_SP)
    annotation (Line(points={{-80,-40},{-48,-40},{-48,-20},{-32,-20}}, color={0,0,127}));
  connect(theta, boiler.theta)
    annotation (Line(points={{-80,40},{-60,40},{-60,20},{-32,20}}, color={0,0,127}));
  connect(boiler.inlet, inlet) annotation (Line(
      points={{-10,32},{-10,76},{-20,76},{-20,90}},
      color={140,56,54},
      thickness=0.5));
  connect(boiler.outlet, outlet) annotation (Line(
      points={{10,32},{10,60},{10,88},{18,88}},
      color={140,56,54},
      thickness=0.5));
  connect(m_flow_SP, PI.REF)
    annotation (Line(points={{-80,0},{-56,0},{-56,52},{-46,52}}, color={0,0,127}));
